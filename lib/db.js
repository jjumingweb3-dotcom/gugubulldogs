  ...
  if (isSupabaseConfigured) {
    try {
      ...
      if (!data || data.length === 0) {
        console.error('No rows were updated in Supabase. Check if RLS policy blocks updates or if the ID exists:', id);
        return false;
      }
      return true;
    } catch (e) {
...
export async function addVideos(newVideos) {
  ...
  // De-duplicate within newVideos itself first based on source_video_id
  const uniqueNewVideos = [];
  const seenIds = new Set();
  for (const v of newVideos) {
    if (v.source_video_id) {
      const vidStr = String(v.source_video_id).trim();
      if (!seenIds.has(vidStr)) {
        seenIds.add(vidStr);
        uniqueNewVideos.push({
          ...v,
          source_video_id: vidStr
        });
      }
    }
  }

  if (uniqueNewVideos.length === 0) return 0;

  if (isSupabaseConfigured) {
    try {
      const sourceVideoIds = uniqueNewVideos.map(v => v.source_video_id);
      ...
      const strictlyNewVideos = uniqueNewVideos.filter(v => 
    ...
    try {
      ...
      let addedCount = 0;
      for (const nv of uniqueNewVideos) {
...
const crawlTargetsFilePath = path.join(process.cwd(), 'data', 'crawl_targets.json');

const defaultCrawlTargets = [
  {
    id: 'target-1',
    platform: 'youtube',
    target_id: 'UCDTAqwh48UIZgczSQYTtFHw',
    team_division: '꿈나무부',
    memo: '꿈나무부 공식 유튜브 채널'
  },
  {
    ...
    target_id: 'pik7688',
    team_division: '새싹부',
    memo: 'U9 새싹부 SOOP 방송국'
  },
  {
    id: 'target-3',
    platform: 'soop',
    target_id: 'ncoolpis1245',
    team_division: '유소년부',
    memo: 'U13 유소년부 SOOP 방송국'
  }
];

function initLocalCrawlTargets() {
  if (isSupabaseConfigured) return;
  const dir = path.dirname(crawlTargetsFilePath);
  if (!fs.existsSync(dir)) {
    fs.mkdirSync(dir, { recursive: true });
  }
  if (!fs.existsSync(crawlTargetsFilePath)) {
    fs.writeFileSync(crawlTargetsFilePath, JSON.stringify(defaultCrawlTargets, null, 2), 'utf-8');
  }
}
...
export async function getCrawlTargets() {
  if (isSupabaseConfigured) {
    try {
      ...
      if (error) {
        console.error('Error fetching crawl targets from Supabase:', error);
        if (error.code === '42P01') {
          return { targets: defaultCrawlTargets, isTableMissing: true };
        }
        return { targets: defaultCrawlTargets, isTableMissing: false };
      }
      // Ensure all targets have an id field (standardized to string)
      const mappedData = (data || []).map(item => ({
        ...item,
        id: String(item.id)
      }));
      return { targets: mappedData, isTableMissing: false };
    } catch (e) {
      console.error('Error communicating with Supabase for crawl targets:', e);
      return { targets: defaultCrawlTargets, isTableMissing: false };
    }
  } else {
    try {
      initLocalCrawlTargets();
      if (!fs.existsSync(crawlTargetsFilePath)) {
        return { targets: defaultCrawlTargets, isTableMissing: false };
      }
      const content = fs.readFileSync(crawlTargetsFilePath, 'utf-8');
      return { targets: JSON.parse(content), isTableMissing: false };
    } catch (e) {
      console.error('Error reading local crawl targets:', e);
      return { targets: defaultCrawlTargets, isTableMissing: false };
    }
  }
}

export async function addCrawlTarget(platform, targetId, teamDivision, memo) {
  if (!platform || !targetId || !teamDivision) return false;
  const trimmedTargetId = targetId.trim();
  const trimmedTeamDivision = teamDivision.trim();
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert([{ 
          ...
          team_division: trimmedTeamDivision, 
          memo: trimmedMemo 
        }])
        .select();
      if (error) {
        console.error('Error adding crawl target to Supabase:', error);
        return false;
      }
      return true;
    } catch (e) {
      console.error('Error communicating with Supabase to add crawl target:', e);
      return false;
    }
  } else {
    try {
      ...
      fs.writeFileSync(crawlTargetsFilePath, JSON.stringify(list, null, 2), 'utf-8');
      return true;
    } catch (e) {
      console.error('Error adding local crawl target:', e);
      return false;
    }
  }
}

export async function deleteCrawlTarget(id) {
  if (!id) return false;
  ...
  if (isSupabaseConfigured) {
    try {
      const { error } = await supabase
        ...
        .eq('id', id);
      if (error) {
        console.error('Error deleting crawl target from Supabase:', error);
        return false;
      }
      return true;
    } catch (e) {
      console.error('Error communicating with Supabase to delete crawl target:', e);
      return false;
    }
  } else {
    try {
      ...
      if (filtered.length !== list.length) {
        fs.writeFileSync(crawlTargetsFilePath, JSON.stringify(filtered, null, 2), 'utf-8');
        return true;
      }
      return false;
    } catch (e) {
      console.error('Error deleting local crawl target:', e);
      return false;
    }
  }
}
