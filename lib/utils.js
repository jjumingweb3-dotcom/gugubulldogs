 ...
 * Format:
 * - With scores: [대회명] 홈팀 이름 홈점수 : 원정점수 원정팀 이름
 * - Without scores: [대회명] 홈팀 이름 vs 원정팀 이름
 */
export function getDisplayTitle(video) {
  if (!video) return '';

  const tournament = video.tournament?.trim() || '';
  const tournamentPrefix = tournament ? `[${tournament}] ` : '';
  
  // Home team name fallback based on division
  const homeTeam = video.home_team?.trim() || 
    (video.team_division && video.team_division !== '미분류' 
      ? `구구불독스 ${video.team_division}` 
      : '구구불독스');
      
  // Away team name fallback based on opponent
  ...
  if (hasHomeScore && hasAwayScore) {
    return `${tournamentPrefix}${homeTeam} ${video.home_score} : ${video.away_score} ${awayTeam}`;
  } else {
    return `${tournamentPrefix}${homeTeam} vs ${awayTeam}`;
  }
}
